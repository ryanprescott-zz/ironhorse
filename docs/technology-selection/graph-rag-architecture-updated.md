...
#### **Graph Database: Pluggable Provider Architecture**

The system implements a **provider abstraction** for graph databases to enable seamless switching between different graph store solutions based on scale, performance, and operational requirements.

**Primary Provider: Neo4j Community Edition** ⭐ RECOMMENDED
- **Why Neo4j**:
  - Industry standard with largest ecosystem (100K+ community members)
  - Cypher query language (intuitive, SQL-like syntax for graphs)
  - Excellent LangChain/LlamaIndex integration (first-class support)
  - Mature visualization tools (Neo4j Browser, Bloom)
  - APOC library for advanced graph operations
  - Production-proven reliability (thousands of enterprise deployments)
  - Native graph storage optimized for traversals (1B relationships in <500ms)
  - Strong Python driver ecosystem
- **Best For**: General-purpose Graph RAG, development, medium-scale production (<50M nodes)
- **License**: GPLv3 (Community Edition) - suitable for internal/SaaS use
- **Deployment**: Single Docker container
- **Query Latency**: 10-100ms for typical graph traversals

**Performance Provider: Memgraph**
- **When to use**: Real-time analytics, streaming data ingestion, performance-critical scenarios
- **Why Memgraph**:
  - 8-50x faster than Neo4j for many workloads
  - In-memory architecture (sub-millisecond queries)
  - Neo4j compatible (Cypher + Bolt protocol)
  - 75% less memory consumption than Neo4j for equivalent datasets
  - Native Kafka/Pulsar streaming connectors
  - C++ implementation (more optimized than Java)
  - Strong Python ecosystem (MAGE library)
- **Best For**: Real-time streaming Graph RAG, write-heavy entity extraction, low-latency requirements (<10ms)
- **License**: Business Source License (Community Edition)
- **Deployment**: Single Docker container, in-memory
- **Query Latency**: 1-10ms for typical graph traversals
- **Limitation**: Dataset must fit in RAM

**Multi-Model Provider: ArangoDB**
- **When to use**: Need unified storage for documents (chunks) AND graphs (entities)
- **Why ArangoDB**:
  - Single database for graph, document, and key-value models
  - Store text chunks as documents alongside entity graphs
  - Unified AQL query language across all models
  - Good graph performance (competitive with Neo4j)
  - Distributed clustering support
  - JSON-native with VelocyPack optimization
- **Best For**: Projects requiring document + graph storage, minimizing database infrastructure
- **License**: ArangoDB Community License (100GB limit, commercial restrictions)
- **Deployment**: Single instance or cluster
- **Query Latency**: 15-50ms for graph traversals

**Massive Scale Provider: NebulaGraph**
- **When to use**: Graphs exceeding 100M nodes, petabyte-scale requirements
- **Why NebulaGraph**:
  - Designed for 100B+ vertices and trillions of edges
  - Excellent performance at scale (better than Dgraph/JanusGraph in benchmarks)
  - Apache 2.0 license (fully permissive)
  - Separation of storage, compute, and meta services
  - nGQL query language (openCypher-based)
  - Balanced data distribution and low storage amplification
  - Cloud-native Kubernetes deployment
- **Best For**: Applications requiring >100M nodes from day one
- **License**: Apache 2.0 (fully open source)
- **Deployment**: Minimum 3-node distributed cluster
- **Query Latency**: 20-100ms for distributed queries
- **Limitation**: Operational complexity, smaller Western ecosystem

**Alternative Provider: JanusGraph**
- **When to use**: Apache 2.0 requirement, existing Cassandra/HBase infrastructure, extreme scale
- **Why JanusGraph**:
  - Apache 2.0 fully open source
  - Pluggable storage backends (Cassandra, HBase, BerkeleyDB)
  - Apache TinkerPop and Gremlin support
  - Massive scale (hundreds of billions of vertices)
  - ACID transactions in distributed environment
  - Apache Spark integration for analytics
- **Best For**: Petabyte-scale graphs, organizations with Cassandra expertise
- **License**: Apache 2.0
- **Deployment**: Distributed (JanusGraph + storage backend + indexing)
- **Query Latency**: 50-200ms for distributed queries
- **Limitation**: Gremlin learning curve, setup complexity

**Design Principle**: Configuration-driven graph database selection enables switching between providers without application code changes. Start with Neo4j for rapid development and mature RAG ecosystem integration. Migrate to alternatives only when specific needs arise:
- **Performance-critical** → Memgraph (if dataset fits in memory)
- **Multi-model** → ArangoDB (unified document + graph storage)
- **Massive scale** → NebulaGraph or JanusGraph (>100M nodes)
- **Apache 2.0 requirement** → NebulaGraph or JanusGraph

#### **Graph Construction: LlamaIndex Property Graph**
- **Why LlamaIndex**:
  - Purpose-built for RAG with graph support
  - Automatic entity and relationship extraction
  - Integration with multiple graph stores (Neo4j, NebulaGraph, etc.)
  - Support for hybrid vector + graph retrieval
  - Works seamlessly with any graph provider through abstraction
- **Entity Extraction Methods**:
  - LLM-based extraction (using Ollama/vLLM - see below)
  - Simple keyword/NER-based extraction
  - Schema-guided extraction for domain-specific use cases

**Graph Schema Design** (Query Language Agnostic):
```cypher
// Nodes (compatible with Cypher, nGQL, Gremlin via provider abstraction)
...
The graph database provider abstraction handles translation between:
- **Cypher** (Neo4j, Memgraph)
- **AQL** (ArangoDB)
- **nGQL** (NebulaGraph)
- **Gremlin** (JanusGraph)

This ensures the same application logic works across all graph databases.
...
- Graph visualization (Cytoscape.js or Pyvis)
- System configuration panel

## Configuration Examples

The pluggable architecture is driven by a central `config.yaml` file that controls all provider selections. Here are examples for different deployment scenarios:

### Development Configuration (Fast Setup)
```yaml
...
embeddings:
  provider: "bge-base"  # Lightweight, CPU-friendly
...
graph_db:
  provider: "neo4j"  # Best RAG ecosystem integration
...
### High-Performance Configuration (Real-Time RAG)
```yaml
# config.yaml - High-performance setup
chunking:
  provider: "semantic"  # Better entity extraction
  chunk_size: 1024
...
graph_db:
  provider: "memgraph"  # 8-50x faster than Neo4j
  host: "localhost"
  port: 7687
  username: "memgraph"
  password: "memgraph"
  # Note: Uses same Bolt protocol as Neo4j

llm:
  provider: "vllm"  # Better throughput
  model: "meta-llama/Meta-Llama-3.1-8B-Instruct"
  api_base: "http://localhost:8000/v1"
```

### Massive Scale Configuration (100M+ Nodes)
```yaml
# config.yaml - Massive scale setup
chunking:
  provider: "semantic"
  chunk_size: 1024
...
vector_db:
  provider: "milvus"  # Scales to billions of vectors
  host: "localhost"
  port: 19530
  collection_name: "documents"

graph_db:
  provider: "nebulagraph"  # Scales to 100B+ nodes
  hosts: 
    - "127.0.0.1:9669"
  username: "root"
  password: "nebula"
...
### Multi-Model Configuration (Unified Document + Graph Storage)
```yaml
# config.yaml - Multi-model setup with ArangoDB
chunking:
  provider: "semantic"
  chunk_size: 1024
...
graph_db:
  provider: "arangodb"  # Stores documents AND graphs
  hosts: "http://localhost:8529"
  username: "root"
  password: "password"
  database: "graph_rag"
  # ArangoDB can store text chunks as documents
  # and entities as graphs in the same database
...
### Apache 2.0 Licensed Stack Configuration
```yaml
# config.yaml - Fully Apache 2.0 licensed stack
chunking:
  provider: "recursive"  # LangChain (MIT)
  chunk_size: 1024

embeddings:
  provider: "bge-base"  # Apache 2.0
  model: "BAAI/bge-base-en-v1.5"

vector_db:
  provider: "milvus"  # Apache 2.0
  host: "localhost"
  port: 19530

graph_db:
  provider: "janusgraph"  # Apache 2.0
  hosts: 
    - "localhost:8182"
  # Requires Cassandra/HBase backend
  storage_backend: "cassandra"
  storage_hostname: "localhost"

llm:
  provider: "vllm"  # Apache 2.0
  model: "meta-llama/Meta-Llama-3.1-8B-Instruct"
  # Note: Model license separate from vLLM license
```

### Provider Factory Pattern

Each component uses a factory pattern to instantiate the configured provider:

```python
# Example: Graph DB Factory Usage
from app.graph_db.factory import GraphDBFactory
from app.config import load_config

# Load configuration
config = load_config("config.yaml")

# Factory automatically selects and instantiates the right provider
graph_db = GraphDBFactory.create(config["graph_db"])

# Unified interface works with any provider
graph_db.add_node(
    label="Entity", 
    properties={"name": "Python", "type": "Technology"}
)

# Query using high-level abstraction
results = graph_db.find_related_entities(
    entity_name="Python",
    relationship_type="RELATED_TO",
    max_depth=2
)

# Query language translation handled by provider:
# - Neo4j/Memgraph: Uses Cypher directly
# - ArangoDB: Translates to AQL
# - NebulaGraph: Translates to nGQL  
# - JanusGraph: Translates to Gremlin
```

### Switching Providers

To switch from Neo4j to Memgraph (for performance):
...
# After: Memgraph (drop-in replacement)
graph_db:
  provider: "memgraph"  # <-- Only change needed
  host: "localhost"
  port: 7687
  username: "memgraph"
  password: "memgraph"
...
No application code changes required - the factory pattern and abstraction layer handle all differences.
...
### Why Pluggable Graph Database Architecture?

**Rationale**:
1. **Ecosystem Maturity**: Start with Neo4j for best RAG integration, migrate for specific needs
2. **Performance Requirements**: Switch to Memgraph when sub-10ms latency required
3. **Scale Evolution**: Migrate to NebulaGraph/JanusGraph when exceeding 100M nodes
4. **Licensing Flexibility**: Move to Apache 2.0 solutions (NebulaGraph, JanusGraph) if needed
5. **Multi-Model Needs**: Switch to ArangoDB for unified document + graph storage
6. **Future-Proofing**: New graph databases can be added without changing core logic

**Implementation Benefits**:
- Switch databases via configuration file (no code changes)
- Query language translation handled by provider abstraction
- All retrieval and construction logic works with any graph DB
- Test different databases on your specific workload
- Vendor independence - not locked into any single graph solution

**Migration Paths**:
- **Neo4j → Memgraph**: Drop-in replacement (same Cypher/Bolt), 8-50x speedup
- **Neo4j → ArangoDB**: Moderate effort, multi-model benefits
- **Neo4j → NebulaGraph/JanusGraph**: Significant effort, massive scale benefits

**Key Insight**: Graph database choice significantly impacts:
- **Query latency**: 1ms (Memgraph) vs 10-100ms (Neo4j) vs 50-200ms (distributed)
- **Write throughput**: Critical for entity extraction during ingestion
- **Scale ceiling**: 50M (Neo4j single-node) vs 100B+ (NebulaGraph/JanusGraph)
- **Operational complexity**: Single container (Neo4j/Memgraph) vs distributed cluster

The pluggable architecture allows you to start with Neo4j for mature RAG ecosystem support and migrate when specific requirements emerge:
- **Need performance** → Memgraph (if data fits in memory)
- **Need scale** → NebulaGraph or JanusGraph (>100M nodes)
- **Need Apache 2.0** → NebulaGraph or JanusGraph
- **Need multi-model** → ArangoDB
...
2. **Neo4j over Alternatives**: Maturity, Cypher language, visualization tools, RAG ecosystem
...
├── docker-compose.memgraph.yml   # Memgraph variant for performance
├── docker-compose.nebulagraph.yml # NebulaGraph variant for massive scale
├── requirements.txt
├── README.md
├── .env.example
...
│   │   ├── neo4j.py              # Neo4j implementation (Cypher)
│   │   ├── memgraph.py           # Memgraph implementation (Cypher)
│   │   ├── arangodb.py           # ArangoDB implementation (AQL)
│   │   ├── nebulagraph.py        # NebulaGraph implementation (nGQL)
│   │   ├── janusgraph.py         # JanusGraph implementation (Gremlin)
...
│   │   └── graph_builder.py      # Graph construction (uses LLM & graph DB factories)
│   ├── retrieval/
│   │   ├── vector_search.py      # Vector DB operations (uses factory)
│   │   ├── graph_search.py       # Graph DB operations (uses factory)
...
└── tests/
    ...
    ├── test_graph_db.py          # Test different graph databases
...
- **Graph Database Comparison**: See companion document for detailed graph DB comparison
...
- **Modularity**: Easy to swap components - text chunking, embeddings, vector database, graph database, and LLM providers
- **Quality**: State-of-the-art models for each component
  ...
  - Neo4j for mature graph RAG ecosystem
  - vLLM for production-scale LLM serving
- **Flexibility**: Quintuple pluggable architecture adapts to different:
  - Document types (markdown, PDFs, code)
  - Processing stages (vector search vs. graph construction)
  - Scale requirements (1M vs. 100M+ vectors/nodes)
  - Resource constraints (development vs. production)
  - Performance needs (real-time vs. batch)
...
- **Production Scalability**: Migrate to optimized options when needed
  ...
  - Memgraph for real-time performance, NebulaGraph for massive scale
...
The combination of vector search (Qdrant/Milvus/Weaviate) and graph database (Neo4j/Memgraph/NebulaGraph/JanusGraph) enables sophisticated retrieval strategies that outperform traditional RAG for complex, entity-rich documents. 

**The quintuple pluggable architecture** ensures you can:
1. **Start simple**: RecursiveChunker + gte-large + Qdrant + Neo4j + Ollama for rapid MVP development
2. **Scale smart**: SemanticChunker + e5-mistral + Milvus + NebulaGraph + vLLM for production performance
3. **Optimize for performance**: Any chunker + any embeddings + Qdrant + Memgraph + vLLM for real-time RAG
4. **Adapt easily**: Change any component via configuration, not code rewrites
5. **Optimize continuously**: A/B test different combinations on your data
...
- **Graph Database**: 8-50x difference in graph query latency (Memgraph vs. Neo4j)
- **LLM Provider**: 2-3x difference in concurrent user capacity (vLLM vs. Ollama)

**Deployment Path**:
- **Weeks 1-2**: Develop with RecursiveChunker + Qdrant + Neo4j + Ollama (baseline)
...
- **As needed**: Migrate to Memgraph for performance or NebulaGraph for scale (>100M nodes)
